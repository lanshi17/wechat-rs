...
macro_rules! auth {
    ($state:expr, $headers:expr) => {{
        let tok = match extract_token(&$headers) {
            Some(t) => t.to_owned(),
            None    => return (StatusCode::UNAUTHORIZED, "missing token").into_response(),
        };
        if !verify_token(&$state.admin_secret, &tok) {
...
struct UpdateConfigReq {
    ...
    wechat_appsecret: Option<String>,
    wechat_encoding_aes_key: Option<String>,
...
struct ConfigResp {
    wechat_token:     String,
    wechat_appid:     String,
    wechat_appsecret_masked: String,
...
async fn login(
    ...
    // 首次启动：hash 为空时允许用环境变量 ADMIN_PASSWORD 直接登录并写入 hash
    let ok = if hash_stored.is_empty() {
        let plain = std::env::var("ADMIN_PASSWORD").unwrap_or_default();
        body.password == plain
    } else {
        verify(&body.password, &hash_stored).unwrap_or(false)
    };
...
pub fn router(state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/stats/detailed", get(stats_detailed))
        .route("/codes",        get(codes_list))
        .route("/users",        get(users))
        .route("/users/search", get(users_search))
        .route("/users/:openid/codes", get(user_codes))
